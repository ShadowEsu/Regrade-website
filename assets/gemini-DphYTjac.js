import{a}from"./index-CwJLnkzT.js";const m=`You are an expert academic assessment analyst. You are being given between 
...
- Return ONLY the JSON. Nothing else.`;async function i(o){var r;try{const e=await o.json();if((r=e.error)!=null&&r.message)return e.error.message;if(e.message)return e.message}catch{}return`Request failed (${o.status})`}const c=2e3,l=90;async function d(o){for(let r=0;r<l;r++){await new Promise(s=>setTimeout(s,c));const e=await a(`/v1/gemini/jobs/${o}`);if(!e.ok)throw new Error(await i(e));const t=await e.json();if(t.status==="done"&&t.result)return t.result;if(t.status==="failed")throw new Error(t.error??"Analysis failed.")}throw new Error("Analysis timed out. Please try again.")}async function h(o,r,e,t){try{const s=await a("/v1/gemini/analyze",{method:"POST",body:JSON.stringify({assignmentData:o,rubricData:r,feedbackData:e,inlineImages:(t==null?void 0:t.inlineImages)??[]})});if(!s.ok)throw new Error(await i(s));if(s.status===202){const{jobId:n}=await s.json();return await d(n)}return s.json()}catch(s){console.error("Gemini Analysis Error:",s);const n=s instanceof Error&&s.message?` (${s.message})`:"";throw new Error(`Analysis failed.${n} Try clearer photos, or paste text from the rubric and feedback.`)}}async function p(o,r){try{const e=await a("/v1/gemini/advocate",{method:"POST",body:JSON.stringify({message:o,history:r})});if(!e.ok)throw new Error(await i(e));const t=await e.json();if(!t.text)throw new Error("Empty response from assistant.");return t.text}catch(e){throw console.error("Advocate API Error:",e),new Error(e instanceof Error?e.message:"Failed to reach the appeal assistant. Try again in a moment.")}}export{m as ANALYTICAL_SYSTEM_PROMPT,p as chatWithAdvocate,h as performComprehensiveAnalysis};
